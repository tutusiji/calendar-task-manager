...
export function TaskBar({
  ...
  const isBeingDragged =
    dragMoveState.isMoving && dragMoveState.task?.id === task.id;
  ...
  const avatarSizeClass =
    taskBarSize === "compact" ? "h-4 w-4" : "h-[22px] w-[22px]";
  const textSizeClass = taskBarSize === "compact" ? "text-xs" : "text-sm";
  const avatarFallbackTextSize =
    taskBarSize === "compact" ? "text-[8px]" : "text-[10px]";
  ...
  return (
    ...
    >
      ...
      >
        {/* 视觉标记：两条竖线 */}
        <div className="flex w-[5px] border-l border-r border-black h-4/5 opacity-80 group-hover/handle:opacity-100">
      ...
      <div className="taskbar flex items-center gap-1 truncate h-full relative z-10">
              ...
              assigneeUsers[0] && (
                <>
                  <Avatar
                    className={cn(
                      avatarSizeClass,
                      "shrink-0 border border-white/30 bg-white"
                    )}
                  >
                    <AvatarImage
                      src={assigneeUsers[0].avatar}
                      alt={assigneeUsers[0].name}
                    />
                    <AvatarFallback
                      className={cn(
                        "bg-white/20 text-white",
                        avatarFallbackTextSize
                      )}
                    >
                ...
                <div className="flex items-center -space-x-1 shrink-0">
                  {assigneeUsers.map((user, index) => (
                    <Avatar
                      ...
                      style={{ zIndex: assigneeCount - index }}
                    >
        ...
        {task.startTime && (
          <span
            className={cn("text-[10px] opacity-90", !showUserInfo && "ml-1")}
          >
            {task.startTime}
          </span>
        )}
        <span
          className={cn(
            "truncate flex-1",
            !showUserInfo && !task.startTime && "ml-1"
          )}
        >
          {task.title}
        </span>
